        ...
        if self.CalcAreaFitOpt == 3:
             ...
             if Jset != -1:
                 ...
                 try:
                     ...
                     ReturnSolution=True
                     Jsimple,p_inner_simple=SSE_outer(params_outer_hat,p2,self.h[r,igoodhw],self.w[r,igoodhw],ReturnSolution,self.sigh,self.sigw,self.Verbose)
                     ...
                     # JW EDITS: In some cases, simple fit explodes due to numerical instability
                     # Check that observations are within reasonable distance of constrainted points
                     # Calculate average width (y) value of fit line
                     # fit_y = piecewise_linear2(self.h[r, igoodhw], *p2)
                     # print(fit_y)
...
def ChooseInitParamsInner(h,w, p2):
    #function to choose initial parameters describing SWOT-like height-width fit
    # If p2 has valid guesses, use to generate init_params_inner
    if len(p2) > 0:
    
        # Extract breakpoints and slopes from p2
        x0 = p2[0]  # first breakpoint (x-value)
        x1 = p2[2]  # second breakpoint (x-value)
        k1 = p2[3]  # slope for the first region
        k2 = p2[4]  # slope for the second region
        k3 = p2[5]  # slope for the third region
        
        # Calculate guesses for intercepts for each region based on width values
        y0 = np.nanmean(w[h < x0])
        y1 = np.nanmean(w[(h >= x0) & (h < x1)])
        y2 = np.nanmean(w[h >= x1])
        
        # Create the initial parameters as a list of slopes and intercepts
        init_params_inner = [k1, y0, k2, y1, k3, y2]
...
def SSE_outer(param_outer,p2,h,w,ReturnSolution,sigh,sigw,Verbose):
    
    [init_params_inner,nparams_inner]=ChooseInitParamsInner(h,w,p2)
